...
SEED = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else (D / "ipad_docs")
index = json.loads((SEED / "_index.json").read_text())
sample_id = index["queue"][0]["id"]
sample = json.loads((SEED / f"{sample_id}.json").read_text())
